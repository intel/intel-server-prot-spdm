...
class pfr_bmc_image(object):
    ...
    def __init__(self, manifest):
        ...
        self.mtd_fname        = os.path.splitext(self.manifest['build_image']['mtd_firmware'])[0]
        print('mtd_fname = {}'.format(self.mtd_fname))
        ...
        for d in self.dict_spi_parts:
            ...
            print('--offset: 0x{:08x}, size: 0x{:08x}, self.pfr_bmc_image_size = 0x{:08x}'.format(d['offset'], d['size'], self.pfr_bmc_image_size))
        self.pfr_bmc_image_size = d['offset'] + d['size']
        ...
        self.csk_prv = os.path.join(os.path.dirname(__file__), 'keys', os.path.dirname(manifest), 'key_csk_prv.pem')
        self.rk_prv  = os.path.join(os.path.dirname(__file__), 'keys', os.path.dirname(manifest), 'key_root_prv.pem')
    ...
    def build_pfr_image(self):
        ...
        scap.set_signed_image("{}-{}_update_capsule.bin".format(self.platform_name, self.mtd_fname))
        scap.sign()
        ...
        fname_a   = "{}-{}_update_capsule.bin".format(self.platform_name, self.mtd_fname)
        fname_o   = "{}-{}-pfr-final.bin".format(self.platform_name, self.mtd_fname)
        fname_src = "{}-image-pfm_signed.bin".format(self.platform_name)
    ...
    def move_temp_file(self):
        ...
        lst_out_file = ["{}_update_capsule.bin".format(self.mtd_fname), "{}-pfr-final.bin".format(self.mtd_fname)]
...
class Redfish_Update:
    ...
    def login(self):
        ...
        try:
            ...
            if self.sess.verify:
                ...
                sys.exit(0)
        slogin = self.post('/redfish/v1/SessionService/Sessions', json=self.auth)
        self.sess.headers.update({'X-Auth-Token': slogin.headers['X-Auth-Token']})
        self.sess_path = slogin.json()['@odata.id']
    ...
    def update(self):
        ...
        update_resp = self.get('/redfish/v1/UpdateService')
        update_uri = update_resp.json()["HttpPushUri"]

        # check for current updates
        progress = self.get('/redfish/v1/TaskService/Tasks')
        for m in progress.json()["Members"]:
            task_id = re.sub(r'/redfish/v1/TaskService/Tasks/([0-9]+)', r'\1', m["@odata.id"])
            task = self.get('/redfish/v1/TaskService/Tasks/{}'.format(task_id))
            tjson = task.json()
            if tjson['Payload']['TargetUri'] == update_uri:
                if tjson['TaskState'] == 'Completed' or \
                    tjson['TaskState'] == 'Exception':
                    continue
        ...
        resp = self.post(update_uri, data=data, headers={"Content-Type":
...
def main(args):
    ...
    if (args.start_build == True) and (args.reference in _list_reference_project):
        ...
        if args.reference == "whit": args.reference = 'whitley'
        if args.reference == "idv": args.reference = 'idaville'
