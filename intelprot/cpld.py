...
class PFR_CPLD(object):
    ...
    :param platform: reference platform name : ['wht', 'egs', 'idv'] or ['whitley', 'eaglestream', 'idaville']
    ...
    def __init__(self, pfr_cfm1_rpd, platform, svn= 0, csk_id=0):
        ...
        if self.pltfrm not in ['whitley', 'eaglestream', 'idaville']:
            raise ValueError
...
class PFR_CPLD_BHS(object):
    """ class for PFR cpld image operation and build and sign update capsule
       Note that, in BHS/KSV PFR, the update capsule has added 16 bytes of OEM data after SVN
       
    :param pfr_cfm1_rpd: pfr_cfm1_rpd file in compile output
    :param svn: SVN number used in update capsule, default is 0
    :param oem_data: OEM unique data = 4 double words, total 16 bytes, default is 0x01010101
    :param csk_id: CSK ID, it is needed csk_id to do signature, default is 0

    """
    def __init__(self, pfr_cfm1_rpd, platform, svn= 0, oem_data = 0x01010101, csk_id=0):
        self.rpd_file = pfr_cfm1_rpd
        if platform == 'bhs': plt = 'birchstream'
        elif platform == 'kvl': plt = 'kaseyville'
        else:
            plt = platform.replace(' ', '').lower()  # remove space and convert to lower case
        self.pltfrm = plt
        print("platform : ", self.pltfrm)
        if self.pltfrm not in ['birchstream', 'kaseyville']:
            raise ValueError
    ...
    def build_update_capsule(self):
        ...
        convert = Convert()
        svn = convert.integer_to_bytes(self.svn, 4)
        oem_data = convert.integer_to_bytes(self.oem_data, 16)
        stream = PFRBitstream()
        stream.initialize()
        stream.append(svn)
        stream.append(oem_data) 
        stream.read(rpd_f)

        # swap the endianness of the bytes
        for i in range(4+16, stream.size(), 4):
            array = stream.get_raw_byte_array()
            array[i], array[i+1], array[i+2], array[i+3] = \
            array[i+3], array[i+2], array[i+1], array[i]

        #reverse the bit order of each byte because we need to do this before writing the RPD into CFM
        #skip svn and oem data in the first 20 bytes
        for i in range(4+16, stream.size()):
            stream.get_raw_byte_array()[i] = reverse_bits(stream.get_raw_byte_array()[i])
...
def main(args):
    ...
    cmdcap.add_argument('-oem', '--oem_data',  metavar="[OEM data 16 bytes]",  dest= 'oem', default = 0x01010101, help='OEM data, 16 bytes, only used in BHS cpld, default is 0x01010101')  
    ...
    if args.action =="build_capsule":
        ...
        if args.plt.lower() in ['bhs', 'birchstream','kvl', 'kayseyville']:
            newcap = PFR_CPLD_BHS(args.rpd, args.plt, args.svn, args.oem, args.csk_id)
        else:
