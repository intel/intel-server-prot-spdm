...
class CFM(object):
    """ Class for CPLD firmware manifest capsule operation
       ...
       Update: if using rpd file, then convert of byte (reverse nibble bit, and swap nibble)
...
def main(args):
    ...
    kcccap.add_argument('-type', '--pctype',  metavar="[PC Type]",  dest='pc_type', help='PC Type for Key Cancellation Certificate, select one from list [cpld_cap, ifwi_pfm, ifwi_cap, bmc_pfm, bmc_cap]. If no type input, the default is to build all types of KCC')
