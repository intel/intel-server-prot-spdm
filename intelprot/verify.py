...
class PFR_IFWI(object):
    ...
    def validate(self):
        ...
        if rcv < os.stat(self.image).st_size:
            ...
            actpfmobj = Verify_PFM(pfm_act)
            rtn  = actpfmobj.validate()
        logger.info("***** verify active pfm:{}".format(rtn))
        ...
        with open(self.image, 'rb') as f:
            for i in lst_hash:
                ...
                if (hashd != i[2]):
                    logger.info('0x{:08x} - 0x{:08x} -- hash: {}, \n calc_hash: {}'.format(i[0], i[1], i[2], hashd))

                verify_hash &= (hashd == i[2])
        rtn &= verify_hash
        logger.info("***** verify active pfm hash data:{}".format(rtn))
        ...
        if self.rk_key is not None:
            ...
            logger.info("**** verify root public key hash provisioning: {}".format(rtn))
        ...
        if self.rk_key is not None and self.csk_prv is not None:
            ...
            logger.info("--------------------------------------------------------")
            if rcv_cap :
                verify_sign2 = BLOCK_1K_Signed(pfm_rcv, self.rk_key, self.csk_prv).validate()
                logger.info("**** verify BLOCK_1K_Signed:pfm_rcv signature:{}".format(verify_sign2))
                logger.info("--------------------------------------------------------")
                rtn = (verify_sign1 and verify_sign2)
                logger.info("**** verify signature: {}=verify_sign1={} and verify_sign2={}".format(rtn, verify_sign1, verify_sign2))
            else:
                logger.info("--------------------------------------------------------")
                rtn = verify_sign1
                logger.info("**** verify signature: {}=verify_sign1={} ".format(rtn, verify_sign1))
        return rtn


class PFR_IFWI_2(object):
    """ PFR IFWI Birch Stream second binary file (64MB size) verification class
    ...
    def validate(self):
        ...
        #else:
        #  logger = logging.getLogger(__name__)
        lst = utility.bin_search_tag(self.image, PFM_MAGIC)
        logger.info("--- list address of PFM_Magic : {} ".format(lst))
        if len(lst) == 1: # only one capsule
            cap_st_addr = int(lst[0], 0) - BLOCK_SIZE*2
            with open(self.image, 'rb') as f:
                f.seek(cap_st_addr+4)
                pc_len  = int.from_bytes(f.read(4), 'little')
                pc_type = int.from_bytes(f.read(1), 'little')
                f.seek(cap_st_addr + BLOCK_SIZE)
                pfm_rcv = f.read(pc_len)
                logger.info("-- cap_st_addr = 0x{:x}, pc_len: 0x{:x}, pc_type=0x{:x}".format(cap_st_addr, pc_len, pc_type))
        elif len(lst) == 2:
            logger.info("--- skip staging capsule ")
            cap_st_addr = int(lst[1], 0) - BLOCK_SIZE*2
            with open(self.image, 'rb') as f:
                f.seek(cap_st_addr+4)
                pc_len  = int.from_bytes(f.read(4), 'little')
                pc_type = int.from_bytes(f.read(1), 'little')
            ...
            logger.error("-- ERROR: No capsule found in the second PFR image !")
            return False

        rcvpfmobj = Verify_PFM(pfm_rcv)
        rtn = rcvpfmobj.validate()
        logger.info("***** verify rcvpfm pfm:{}".format(rtn))
        ...
        if self.rk_key is not None and self.csk_prv is not None:
            ...
            logger.info("**** verify BLOCK_1K_Signed:pfm_rcv signature:{}".format(verify_sign))
            logger.info("--------------------------------------------------------")
            rtn = verify_sign
            logger.info("**** verify signature: {}=verify_sign={}".format(rtn, verify_sign))
        return rtn

class Check_PFR_IFWI(object):
    """ check PFR IFWI binary file class """
    def __init__(self, ifwi_image):
        self.image = ifwi_image

    def if_first_ifwi(self):
        """ check if it is first ifwi """
        # check if start with signature 5a a5 f0 0f
        with open(self.image, 'rb') as f:
            self.bdata_20 = f.read(20)
        if self.bdata_20.hex() == "ff"*16 + "5aa5f00f":
            logger.info("--- First PFR image ")
            return True
        else:
            return False

    def if_second_ifwi(self):
        """ check if it is BHS second ifwi with staging capsule and recovery capsule """
        lst1=utility.bin_search_tag(self.image, BLK0_MAGIC)
        lst2=utility.bin_search_tag(self.image, PFM_MAGIC)
        if (len(lst1) >= 1) and (len(lst2) >=1 ) and (not self.if_first_ifwi()):
            logger.info("--- Second PFR image ")
            return True
...
class BLOCK_1K_Signed(object):
    ...
    def verify_pc_hash(self):
        ...
        logger.info("***-- pc_data hex str: {}, pclen={}".format(self.pc_data[0:0x100].hex(), hex(pclen)))
    ...
    def verify_signature(self):
        ...
        """
        logger.info("----verify_dignature csk----")
        rtn = True
        if keys.get_eckey_type(self.rk_key) == 'private':
        ...
        logger.info("----verify_dignature blk0 ----")
...
def main(args):
    ...
    if args.action == 'ifwi':
        if Check_PFR_IFWI(args.input_bin).if_first_ifwi():
            obj=PFR_IFWI(args.input_bin, args.rk_key, args.csk_prv, args.logfile)
        if Check_PFR_IFWI(args.input_bin).if_second_ifwi():
            obj=PFR_IFWI_2(args.input_bin, args.rk_key, args.csk_prv, args.logfile)
        rtn = obj.validate()
        result="PASS" if rtn==True else "FAILED"
