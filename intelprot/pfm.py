...
PFM_BHS_HASH_SIZE= 0x30 # 48 bytes for BHS PFM hash
BLOCK_SIGN_SIZE  = 0x400 # block sign size 1K
...
AFM_PER_DEVICE_FMT = '<16sI8sH16sBBBHBBBHIHH18sH48s48sIHHB3s'
AFM_PER_DEVICE_KEY = ('UUID', 'PlatID', 'PlatModel', 'PlatVer', 'rsvd1', 'BusID', 'DevAddr', 'BindSpec', 'BindSpecVer', \
                      'Policy', 'SVN', 'rsvd2', 'AFMVer', 'CurveMagic', 'ManufStr', 'ManufModel', 'rsvd3', 'PubKeySize', 'PubKeyX', 'PubKeyY', \
                      'PubExp', 'rsvd4', 'CertSize', 'TotalMeas', 'rsvd5')

AFM_PER_MEAS_FMT = '<B3sBBH'
AFM_PER_MEAS_KEY = ('NumMeas', 'rsvd1', 'MeasIdx', 'MeasType', 'MeasSize')

dict_MEAS_IDX_CONTENT = { \
    1:"SoC Boot Time FW",\
    2:"Platform Strap Configuration",\
    3:"FIT record 4",\
    4:"uCode FIT patch",\
    5:"Startup ACM",\
...
class BHS_PFM(object):
    ...
    def __init__(self, image_bdata):
        ...
        #print("-- Total_size: {}".format(total_size))
        count = 0x20
        spi_idx, smb_idx, fvm_idx, afm_idx = 0, 0, 0, 0
        loop_cnt = 0
        while (count < total_size) & (loop_cnt < MAX_LOOP):
            pfm_def_type = int(struct.unpack("<B", self.pfm_bdata[count:count+1])[0])

            if pfm_def_type == PFM_SPI_REG_DEF: #0x1 - SPI:
                lst_temp = struct.unpack(PFM_BODY_SPI_FMT, self.pfm_bdata[count:count+struct.calcsize(PFM_BODY_SPI_FMT)])
                for (k, v) in zip(PFM_BODY_SPI_KEY, lst_temp):
                    self.pfm_dict['spi']['{}'.format(spi_idx)][k] = v
                hash_info = self.pfm_dict['spi']['{}'.format(spi_idx)]['hash_alg']
                count += 0x10
                if hash_info == 0:
                    self.pfm_dict['spi']['{}'.format(spi_idx)]['hash_data'] = ''
                if hash_info == 2:
                    self.pfm_dict['spi']['{}'.format(spi_idx)]['hash_data'] = struct.unpack('48s', self.pfm_bdata[count:count+PFM_BHS_HASH_SIZE])[0].hex()
                    count += 48
                spi_idx += 1

            if pfm_def_type == PFM_SMB_RUL_DEF: #0x2 - SMB SPI:
                lst_temp = struct.unpack(PFM_BODY_SMB_FMT, self.pfm_bdata[count:count+struct.calcsize(PFM_BODY_SMB_FMT)])
                for (k, v) in zip(PFM_BODY_SMB_KEY, lst_temp):
                    self.pfm_dict['smb']['{}'.format(smb_idx)][k] = v
                smb_passlist = self.pfm_dict['smb']['{}'.format(smb_idx)]['smb_passlist']
                self.pfm_dict['smb']['{}'.format(smb_idx)]['smb_passlist'] = smb_passlist.hex()
            ...
            lst_temp=[]
            if pfm_def_type == PFM_FVM_SPI_DEF: #0x3 - FVM SPI:
                lst_temp = struct.unpack(PFM_BODY_FVM_FMT, self.pfm_bdata[count:count+struct.calcsize(PFM_BODY_FVM_FMT)])
                for (k, v) in zip(PFM_BODY_FVM_KEY, lst_temp):
                    self.pfm_dict['fvm']['{}'.format(fvm_idx)][k] = v
                count += struct.calcsize(PFM_BODY_FVM_FMT)
                fvm_idx += 1

            if pfm_def_type == PFM_AFM_SPI_DEF: #0x5 - AFM SPI:
                lst_temp = struct.unpack(PFM_BODY_AFM_FMT, self.pfm_bdata[count:count+struct.calcsize(PFM_BODY_AFM_FMT)])
                for (k, v) in zip(PFM_BODY_AFM_KEY, lst_temp):
                    self.pfm_dict['afm']['{:d}'.format(afm_idx)][k] = v
                count += struct.calcsize(PFM_BODY_AFM_FMT)
                afm_idx += 1
            loop_cnt += 1
        ...
        self.smb_cnt = smb_idx
        self.fvm_cnt = fvm_idx
        self.afm_cnt = afm_idx

        # process FVM data
        for i in range(0, fvm_idx):
            fvm_dict = self.pfm_dict['fvm']['{}'.format(i)]
            fvm_addr = fvm_dict['fvm_addr'] #- (fvm_dict['fvm_addr'] & 0xFFFF0000) #0x3FE0000 #(pfm_start-0x400) #PFR_PFM_OFFSET
            fvm_len  = struct.unpack('<I', self.bdata[(fvm_addr+4):(fvm_addr+8)])[0]
            fvm_dict['bdata'] = self.bdata[fvm_addr+BLOCK_SIGN_SIZE: (fvm_addr+BLOCK_SIGN_SIZE+fvm_len)]
            ...
            while fvm_spi_def == 0x01:
                ...
                if fvm_dict['spi']['{:d}'.format(fvm_spi_idx)]['hash_alg'] == 0x02:
                    fvm_dict['spi']['{:d}'.format(fvm_spi_idx)]['hash_data'] = struct.unpack('48s', fvm_dict['bdata'][addr_offset:(addr_offset+48)])[0].hex()
                    #print("--addr_offset=0x{:08x}".format(addr_offset))
                    #print(fvm_dict['bdata'][addr_offset:(addr_offset+48)].hex())
                    addr_offset += 48
        ...
        # process AFM data, only one AFM inside CPU IFWI
        for i in range(0, afm_idx):
            afm_dict = self.pfm_dict['afm']['{}'.format(i)]
            #print("self.pfm_dict['afm'][{}]={}".format(i, self.pfm_dict['afm'][i]))
    ...
    def process_fvm(self):
        ...
        for i in range(0, self.fvm_cnt):
            ...
            for idx in range(0, spi_cnt):
                lst1 = []
                dic = self.pfm_dict['fvm']['{:d}'.format(i)]['spi']['{:d}'.format(idx)]
                #print('0x{:08X} -- 0x{:08X} -- hash_data: {}'.format(dic['reg_start'], dic['reg_end'], dic['hash_data']))
                lst1.append('0x{:08x}'.format(dic['reg_start']))
                lst1.append('0x{:08x}'.format(dic['reg_end']))
                lst1.append('{}'.format(dic['hash_data']))
                lst_v.append(lst+lst1)
    ...
    def process_afm(self):
        """ process AFM for display AFM in the pfr image """
        if self.no_pfm_tag: return
        if self.afm_cnt == 0: return

        self.afm_header = ('Type', 'DeviceAddr', 'Rsvd1', 'UUID', 'PlatformID', 'PlatformModel', 'PlatformVersion', 'Rsvd2', 'Length', 'Address')
        self.afm_value = []
        for k in PFM_BODY_AFM_KEY:
            if k in ['uuid', 'platform_model', 'rsvd2']:
                val=self.pfm_dict['afm']['0'][k].hex()
            else:
                val=hex(self.pfm_dict['afm']['0'][k])
            self.afm_value.append(val)
        self.afm_dict  = ConfigDict() #self.pfm_dict['afm']['0']
        self.afm_addr  = self.pfm_dict['afm']['0']['afm_addr']
        self.afm_len   = struct.unpack('<I', self.bdata[(self.afm_addr+4):(self.afm_addr+8)])[0]
        self.afm_bdata = self.bdata[(self.afm_addr+BLOCK_SIGN_SIZE):(self.afm_addr+BLOCK_SIGN_SIZE+self.afm_len)]
        afm_per_dev_s = struct.calcsize(AFM_PER_DEVICE_FMT)
        lst_temp = struct.unpack(AFM_PER_DEVICE_FMT, self.afm_bdata[0:afm_per_dev_s])

        for (k, v) in zip(AFM_PER_DEVICE_KEY, lst_temp):
            self.afm_dict[k] = v
        self.total_meas = self.afm_dict['TotalMeas']
        afm_per_meas_s = struct.calcsize(AFM_PER_MEAS_FMT)
        start_idx = afm_per_dev_s
        for i in range(0, self.total_meas):
            lst_temp = struct.unpack(AFM_PER_MEAS_FMT, self.afm_bdata[start_idx:(start_idx + afm_per_meas_s)])
            for (k, v) in zip(AFM_PER_MEAS_KEY, lst_temp):
                self.afm_dict['meas']['{:d}'.format(i)][k] = v
            meas_size = self.afm_dict['meas']['{:d}'.format(i)]['MeasSize']
            self.afm_dict['meas']['{:d}'.format(i)]['MeasValue'] = self.afm_bdata[(start_idx + afm_per_meas_s):(start_idx + afm_per_meas_s + meas_size)]
            start_idx += afm_per_meas_s + meas_size
    ...
    def show_afm_meas(self, fmt='orgtbl'):
        """ display measurement index idx """
        lst_afm_meas_header = ['MeasIdx', 'Content', 'MeasType', 'MeasSize', 'MeasValue']
        lst_afm_meas_value  = []
        for i in range(0, self.total_meas):
            lst_temp  = []
            for k in lst_afm_meas_header:
                if k == 'MeasValue': self.afm_dict['meas']['{:d}'.format(i)][k] = self.afm_dict['meas']['{:d}'.format(i)][k].hex()
                elif k == 'Content':
                    self.afm_dict['meas']['{:d}'.format(i)][k] = dict_MEAS_IDX_CONTENT[int(self.afm_dict['meas']['{:d}'.format(i)]['MeasIdx'], 0)]
                else:
                    self.afm_dict['meas']['{:d}'.format(i)][k] = hex(self.afm_dict['meas']['{:d}'.format(i)][k])
                lst_temp.append(self.afm_dict['meas']['{:d}'.format(i)][k])
            lst_afm_meas_value.append(lst_temp)
        msg = '---- Measurements in AFM ----\n'
        msg += tabulate.tabulate(lst_afm_meas_value, lst_afm_meas_header, tablefmt=fmt)
        #msg += '\n-----------------------------------\n'
        logger.info('\n'+msg)


    def show_afm_dev(self, fmt='orgtbl'):
        """ display per device afm except meas"""
        self.afm_per_dev_header = ('UUID', 'PlatID', 'PlatModel', 'PlatVer', 'BusID', 'DevAddr', 'BindSpec', 'BindSpecVer', \
                          'Policy', 'SVN', 'AFMVer', 'CurveMagic', 'ManufStr', 'ManufModel', 'PubKeySize', 'PubKeyX', 'PubKeyY', 'TotalMeas')
        self.afm_per_dev_value = []
        msg = '\n-----------------------------------------\n'
        for k in self.afm_per_dev_header:
            if k in ['UUID', 'PlatModel', 'PubKeyX', 'PubKeyY']:
              v = self.afm_dict[k].hex()
            elif k == 'CurveMagic': v = hex(self.afm_dict[k])
            else: v = self.afm_dict[k]
            self.afm_per_dev_value.append(v)
            msg += '{}: {}\n'.format(k, v)
        #msg = tabulate.tabulate([self.afm_per_dev_value], self.afm_per_dev_header, tablefmt=fmt)
        msg += '-----------------------------------------\n'
        logger.info('\n'+msg)


    def show_afm(self, fmt='orgtbl'):
        """ display FVM in the pfr image """
        self.process_afm()
        if (self.no_pfm_tag) or (self.fvm_cnt == 0): return
        msg = '\n================ CPU AFM ================\n'
        msg += tabulate.tabulate([self.afm_value], self.afm_header, tablefmt=fmt)
        logger.info('\n'+msg)
        self.show_afm_dev()
        self.show_afm_meas()
